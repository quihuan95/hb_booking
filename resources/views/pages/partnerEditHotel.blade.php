...
<html lang="vi">
   <head>
	...
	<style>
		...
		label.term-item{ width: 100%;clear: both; }
		.text-right{ display: block;margin-top: 10px; }
		.g-items .item{ width: 100%;margin-bottom: 10px; }
	</style>
   </head>
   <body>
   <?php
...
<div class="__p3ch" style="">
   <span style="color: #04245d;margin-right: 10px;font-size: 20px;" class="fleft"><a href="{!! route('home.cmscpanel') !!}"><i style="display: block;" class="fa fa-arrow-circle-o-left" aria-hidden="true"></i></span> <span class="fleft fsize13">Khách sạn</span></a> <span class="__p4ch"></span> <span class="fleft fsize13">Chỉnh/sửa khách sạn</span>
</div>
...
<?php
	...
	if($qqq!=''){
		if($qqq==1){
			...
			$tab1='';$tab2=' active';$tab3='';$tab4='';$tab5='';
		}else if($qqq==3){
			$tab1='';$tab2='';$tab3=' active';$tab4='';$tab5='';
		}else if($qqq==4){
			$tab1='';$tab2='';$tab3='';$tab4=' active';$tab5='';
		}else if($qqq==5){
			$tab1='';$tab2='';$tab3='';$tab4='';$tab5=' active';
		}
	}
	?>
...
<div class="row">
  <div class="col-2">
	<div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
	  <a class="nav-link  <?php echo $tab1;?>" id="v-pills-home-tab" data-toggle="pill" href="#v-pills-home" role="tab" aria-controls="v-pills-home" aria-selected="true">Thông tin chung</a>
	  <a class="nav-link  <?php echo $tab2;?>" id="v-pills-settings-tab" data-toggle="pill" href="#v-pills-settings" role="tab" aria-controls="v-pills-settings" aria-selected="false">Thư viện ảnh</a>
	  <a class="nav-link  <?php echo $tab3;?>" id="v-pills-profile-tab" data-toggle="pill" href="#v-pills-profile" role="tab" aria-controls="v-pills-profile" aria-selected="false">Chính sách</a>
	  @if($data->status=='publish')
	  <a class="nav-link  <?php echo $tab4;?>" id="v-pills-messages-tab" data-toggle="pill" href="#v-pills-messages" role="tab" aria-controls="v-pills-messages" aria-selected="false">Thiết lập phòng</a>
	  <a class="nav-link  <?php echo $tab5;?>" id="v-setting-room-tab" data-toggle="pill" href="#v-setting-room" role="tab" aria-controls="v-setting-room" aria-selected="false">Điều chỉnh tình trạng phòng trống</a>
	  @endif
  ...
  <div class="col-10">
	<div class="tab-content" id="v-pills-tabContent">
	  ...
	  <form action="{!! route('hotel.update.partnership') !!}" method="post" id="form11" name="form11"  enctype="multipart/form-data">
	{{ csrf_field() }}
	...
	<input type="hidden" name="r_status" id="r_status" value="draft" />
	  <div class="row">
	  <div class="col-md-9">
<div class="panel">
...
<div class="">
<textarea name="content" class="d-none has-ckeditor my-editor" cols="30" rows="10">{!! $data->content !!}</textarea>
</div>
</div>
<div class="form-group">
...
<script type="text/javascript">
	  ...
	  <div class="tab-pane fade  <?php echo 'show '.$tab2;?>" id="v-pills-settings" role="tabpanel" aria-labelledby="v-pills-settings-tab">
...
<div class="text-center">
<form action="{!! route('ajaxupload.avatar') !!}" method="post" id="form11" name="form11"  enctype="multipart/form-data">
{{ csrf_field() }}
<input type="hidden" value="{{ $data->id }}" id="id_hotel_avatar" name="id_hotel_avatar">
<input type="hidden" value="draft" id="avatar_status" name="avatar_status">
...
{!! \App\Models\FileHelper::isImgs($data->image_id) !!}
</label>
@else
<div class="text-center">
...
<form action="{!! route('ajaxupload.doGallery') !!}" method="post" id="form12" name="form12"  enctype="multipart/form-data">
{{ csrf_field() }}
<input type="hidden" value="{{ $data->id }}" id="id_hotel2" name="id_hotel2"/>
<input type="hidden" value="draft" id="gallery_status" name="gallery_status">
...
{!! \App\Models\FileHelper::imgGallery($data->gallery,$data->id,'0') !!}
@endif
...
<div class="text-right">
	<a class="btn btn-primary" href="{!! route('cmscpanel.editHotel',array($data->id)) !!}?tab=3"><i class="fa fa-save"></i> Lưu & Tiếp tục</a>
	<p>&nbsp;</p>
	<p>&nbsp;</p>
	<p>&nbsp;</p>
	<p>&nbsp;</p>
	<p>&nbsp;</p>
	  ...
	  <div class="tab-pane fade <?php echo  'show '.$tab4;?>" id="v-pills-messages" role="tabpanel" aria-labelledby="v-pills-messages-tab">
...
<input type="hidden" value="draft" id="calendars_status" name="calendars_status">
<div class="panel">
...
<option value="1 giường đôi và 1 giường Queen">1 giường đôi và 1 giường Queen</option>
...
<td class="title">
<a href="{!! route('cmscpanel.editRoom',array($data->id,$v6->id)) !!}">{{$v6->title}}</a>
</td>
<td>
{{$v6->bed_type}}
</td>
...
<td>
 <a href="{!! route('cmscpanel.editRoom',array($data->id,$v6->id)) !!}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> Sửa
</a>

<a class="btn btn-danger btn-sm"  onclick="return confirm('Bạn có thật sự muốn xóa?')" href="{!! route('room.delete.partnership',array($data->id,$v6->id)) !!}"><i class="fa fa-trash-o" aria-hidden="true"></i> Xóa</a>
</td>
</tr>
@endforeach
 </tbody>
...
<p>&nbsp;</p>      
	  ...
	  <div class="tab-pane fade <?php echo  'show '.$tab3;?>" id="v-pills-profile" role="tabpanel" aria-labelledby="v-pills-profile-tab">
	...
	<div class="row" style="margin-bottom:15px;">
	  ...
	  <div class="row" style="margin-bottom:15px;">
	  <input type="submit" class="btn btn-primary" id="btnSave1" name="btnSave1" value="Lưu lại"/>
	  <input style="margin-left:10px;" type="buttom" class="btn btn-warning" id="btnSave2" name="btnSave2" value="Bỏ qua"/>
	  </div>
	  ...
	  <p style="text-align:center;">
		<a style="font-size: 20px;padding: 5px 35px;width:100%;" class="btn btn-success btn-sm"  href="{!! route('parter.complate',array($data->id)) !!}"><i class="fa fa-paper-plane" aria-hidden="true"></i> GỬI XÉT DUYỆT -  HOÀN THÀNH ĐĂNG KÝ</a>
	  </p>
	  ...
	  <div class="tab-pane fade  <?php echo 'show '.$tab5;?>" id="v-setting-room" role="tabpanel" aria-labelledby="v-setting-room-tab">
		<h2>Lịch</h2>
		<div class="row">
		<div class="col-md-12">
...
<p>&nbsp;</p>
		...
		<div class="row" style="margin-top:15px;">
			...
			<td>
			<button style="margin-top: 20px;margin-left: -65px;" onclick="fnSearch()" class="btn btn-primary" name="btnSearch" id="btnSearch">
		...
		<script type="text/javascript">
			...
			function fnSearchClick(v){
				...
				function search(){
					$('#v_save_room').html('<input type="button" name="btnSaveRoom" onclick="fnSaveRoom()" id="btnSaveRoom" class="btn btn-primary" value="Lưu thay đổi">');
				...
				function fnSave(){
					...
					if(str_info!=''){
						$.ajax({
							url:"{!! route('room.update.partnership.calendar') !!}",
							method:"POST",
				...
				function fnSaveMultiRoom(){
					...
					//$('#v_save_room').html('<span class="btn btn-primary">Processing...</span>');
					var checkin=document.getElementsByClassName("t-input-check-in")[0].value;
					var checkout=document.getElementsByClassName("t-input-check-out")[0].value;
					var dateParts = checkin.split("-");
					var dateParts2 = checkout.split("-");
					...
					var valStatus='A';
					//if(rdoStatus[0].checked == true){ valStatus='A'; }
					//else{ valStatus='N'; }
						...
						if(checkDayCurrent(days2)){
							str_info+=$('#phongdeban_'+days2).val()+'v'+$('#standard_rate_'+days2).val()+'v'+$('#refundable_'+days2).val()+'v'+days2+'v'+$('#status_room_'+days2).val()+'z';
								...
								if(fnCheckThu(current_thu)){
									...
									str_info+=$('#soluongphongmulti').val()+'v'+$('#pricePhongMulti').val()+'v'+''+'v'+days2+'v'+valStatus+'z';//$('#refundable_'+days2).val()
		...
		.boxhname{ width:50%;float: left;padding: 0 10px;/*line-height: 68px;*/ }
		</style>
...
function fnSaveRoom(){
	...
	if(rdoStatus[0].checked === false && rdoStatus[1].checked === false){
		alert('Vui lòng lựa chọn tình trạng mở/đóng phòng');
		$('#rdoMoPhong').focus();
		return;
	}
...
<p>
<span id="v_save_room">
	<input type="button" name="btnSaveRoom" onclick="fnSaveRoom()" id="btnSaveRoom" class="btn btn-primary" value="Lưu thay đổi" />
</span>
...
<div style="width:100%;height:1px;clear:both;"></div>
	...
	<script src="{{ asset('dist/admin/js/manifest.js?_ver=1.4.0') }}"></script>
	<script src="{{ asset('dist/admin/js/vendor.js?_ver=1.4.0') }}"></script>
	<script src="{{ asset('dist/admin/js/app.js?_ver=1.4.0') }}"></script>
