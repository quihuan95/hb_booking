...
<html lang="vi">
   <head>
	...
	<style>
		...
		._wap56{ position: absolute;float: left;width: 100%;background: url(https://hoabinhbooking.test/icon/bgo.png) repeat top left;bottom: 0px;text-align: left;padding: 5px;overflow: hidden;border-bottom-left-radius: 5px;border-bottom-right-radius: 5px;color:#fff; }
...
<div class="_wap11">
	<a href="{!! route('cmscpanel.createHotel') !!}" class="btn btn-primary _wap12"><i class="fa fa-plus-circle" aria-hidden="true"></i> Tạo mới chỗ nghỉ</a>
</div>
...
@if(!empty($dt_draft))
	<div class="___hotel_wait_ab">
		...
		<?php $i++;?>
			{!! \App\Models\FileHelper::isImgsCpanel($v5->image_id,$v5->id) !!}
		@endforeach
		<?php if($i==0){ ?>
			<p>&nbsp;</p>
			<p>&nbsp;</p>
...
@if(!empty($dt_publish))
		...
		@foreach($dt_publish as $k6=>$v6)
			{!! \App\Models\FileHelper::isImgsCpanel($v6->image_id,$v6->id) !!}
		@endforeach
		</div>
@endif
