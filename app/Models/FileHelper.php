...
class FileHelper extends Model
{
	public static function isImgsCpanel($oId,$oHotelId){
		...
		if($data){
			...
			$data->file_path = preg_replace('/^public\//', '', $data->file_path);
			if($dtHotel->status=='publish'){
				return '<div class="_wap55"><a href="'.$link_edit.'?tab=1"><img alt="" src="https://hoabinhbooking.test/'.$data->file_path.'"/></a><div class="_wap56">'.$dtHotel->title.'</div><div class="_wap57"><a onclick="return confirm(\'Are you sure want delete?\');" href="javascript:void(0);"><i class="fa fa-trash" aria-hidden="true"></i></a></div></div>';
	...
	public static function isImgsSearch($oId,$title){
		...
		if($data){
			$dd=substr($data->file_path,0,19);//uploads/img/
			$data->file_path = preg_replace('/^public\//', '', $data->file_path);
			if($dd=='uploads/img/'){
				return '<img class="img-responsive lazy loaded" data-src="https://hoabinhbooking.test/'.$data->file_path.'" alt="'.$title.'" src="https://hoabinhbooking.test/'.$data->file_path.'" data-was-processed="true">';
			}else{
				return '<img class="img-responsive lazy loaded" data-src="https://hoabinhbooking.test/uploads/'.$data->file_path.'" alt="'.$title.'" src="https://hoabinhbooking.test/uploads/'.$data->file_path.'" data-was-processed="true">';
			}
		...
		foreach($mang as $k => $v)
		{
			if($v!=''){
				...
				if(!empty($one_img)){
					...
					$_re.='<div class="inner"><a href="https://hoabinhbooking.test/vi/partnership/public/gallery/delete/'.$hotelId.'/'.$v.'/'.$status.'"><span class="delete btn btn-sm btn-danger"><i class="fa fa-trash"></i></span></a><img src="/'.$one_img->file_path.'" class="image-responsive"></div>';
					$_re.='</div>';
				}
			}
		}
		...
		foreach($mang as $k => $v)
		{
			...
			if($v!=''){
				if($count<=4){
					$one_img=DB::table('media_files')->where('id',$v)->first();
					if(!empty($one_img)){
						$one_img->file_path = preg_replace('/^public\//', '', $one_img->file_path);
						if($count==4){
