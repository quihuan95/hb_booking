...
class MenuWalker
{
    ...
    public function generateTree($items = [])
    {
        ...
        foreach ($items as $item) {
            ...
            $rel='';
            if($item['name']=='Vé máy bay' || $item['name']=='Tours' || $item['name']=='Blog' || $item['name']=='Voucher du lịch' || $item['name']=='Liên hệ'){ $rel=' rel="nofolow"'; }else{ $rel=''; }
            printf('<a '.$rel.' target="%s" href="%s" >%s</a>', $item['target'], $url, $item['name']);
            if (!empty($item['children'])) {
